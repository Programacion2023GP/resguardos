...
export class ReguardsComponent {
  ...
  editing: any;
  brand =
    localStorage.getItem('brand') !== null ? localStorage.getItem('brand') : ''; //marca
  type =
    localStorage.getItem('type') !== null ? localStorage.getItem('type') : ''; //tipo
  state =
    localStorage.getItem('state') !== null ? localStorage.getItem('state') : ''; //estado fisico
  ...
  serial =
    localStorage.getItem('serial') !== null
      ...
      : ''; //serial
  airlne =
    localStorage.getItem('airlne') !== null
      ? localStorage.getItem('airlne')
      : ''; //aerea de adscripcion
  numberNomina =
    localStorage.getItem('numberNomina') !== null
      ? localStorage.getItem('numberNomina')
      : ''; //aerea de adscripcion
  ...
  constructor(
    ...
    this.cols = [
      ...
      },
      {
        field: 'description',
        header: 'DESCRIPCIÓN',
        customExportHeader: 'DESCRIPCIÓN',
      },
      {
        field: 'brand',
        header: 'MARCA Y MODELO',
        customExportHeader: 'MARCA Y MODELO',
      },
      {
        field: 'Tipo',
        header: 'TIPO DE RESGUARDO',
        customExportHeader: 'TIPO DE RESGUARDO',
      },
      {
        field: 'serial',
        header: 'NUMERO SERIAL',
        customExportHeader: 'NUMERO SERIAL',
      },
      {
        field: 'number_korima',
        header: 'NUMERO DE KORIMA',
        customExportHeader: 'NUMERO DE KORIMA',
      },
      ...
      {
        field: 'motive',
        header: 'MOTIVO DADO DE BAJA',
        customExportHeader: 'MOTIVO DADO DE BAJA',
      },
      {
        field: 'observations',
        header: 'OBSERVACIONES',
        customExportHeader: 'OBSERVACIONES',
      },
    ];
  ...
  deleteResguard(guard:any){
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.service.Data<any>(`guardshabilited/${guard.id}`).subscribe({
          next: (n: any) => {
            this.Toast.fire({
              position: 'top-end',
              icon:'success',
              title: 'Resguardo disponible para asignar',
            });
            this.getGuards();
            ...
            this.Toast.fire({
              position: 'top-end',
              icon:'success',
              title: 'El resguardo no se puede poner en dispocisión para asignar ocurrio un error',
            });
            this.getGuards();
  ...
  GetDataGroups() {
    this.service
      .OtherData<any>(
        `https://declaraciones.gomezpalacio.gob.mx/nominas/departamentos/infraesctruturagobmxpalaciopeticioninsegura`
      )
      .subscribe({
        next: (n: any) => {
  ...
  onSearch(event: Event): void {
    ...
    this.filteredOptions = this.groups.filter((option) =>
      option.departamento.toLowerCase().includes(searchText)
    );
  }
  onSearch2(event: Event): void {
    const searchText = (event.target as HTMLInputElement).value.toLowerCase();

    this.filteredOptions2 = this.types.filter((option) =>
  ...
  onSearch3(event: Event): void {
    ...
    this.filteredOptions3 = this.states.filter((option) =>
      option.name.toLowerCase().includes(searchText)
    );
  }
  selectOption(option: Option): void {
    ...
    const exist = this.groups.filter(
      (item) => item.departamento == option.departamento
    );
    if (exist) {
  ...
  exportExcel() {
    import('xlsx').then((xlsx) => {
      ...
      const modifiedGuards = this.guardSave.map(
        (guard: { [x: string]: any }) => {
  ...
  searchEmployeed(event: any) {
    this.service
      .OtherData<any>(
        `https://declaraciones.gomezpalacio.gob.mx/nominas/empleados/${event.target.value}/infraesctruturagobmxpalaciopeticioninsegura`
      )
      .subscribe({
        next: (n: any) => {
          ...
          this.myFormUpdate
            .get('employeed')
            ?.setValue(
              `${employed.nombreE} ${employed.apellidoP} ${employed.apellidoM}`
            );
